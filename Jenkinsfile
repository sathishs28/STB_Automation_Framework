pipeline {
    ...
    stages {
        ...
        stage('Wait for Analysis Processing') {
            steps {
                // Wait a few seconds for SonarQube to process the report
        ...
        stage('Fetch SonarQube Metrics') {
            steps {
                script {
                    ...
                    def response = sh(
                        script: """
                            curl -s -w "\\nHTTP_CODE:%{http_code}" -u ${env.SONAR_AUTH_TOKEN}: "${apiUrl}"
                        """,
                        returnStdout: true
                    ).trim()
                    ...
                    if (httpCode == "200" && body) {
                        try {
                            // Parse JSON manually without readJSON plugin
                            ...
                            def getMetric = { metricName ->
                                def m = measures.find { it.metric == metricName }
                                return m?.value ?: '0'
                            }
                            ...
                            echo "Metrics: Bugs=${env.SONAR_BUGS}, Vulns=${env.SONAR_VULNERABILITIES}, Smells=${env.SONAR_CODE_SMELLS}, Coverage=${env.SONAR_COVERAGE}"

                        } catch (Exception parseError) {
                            echo "ERROR parsing JSON: ${parseError.message}"
                            setDefaultMetrics()
                        }
                    } else {
                        echo "ERROR: API call failed with HTTP ${httpCode}"
                        setDefaultMetrics()
                    }
                }
            }
